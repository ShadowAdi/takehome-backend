...
class AssessmentSubmissionServiceClass {
    ...
    public async submitAssessmentSubmission(payload: AssessmentSubmissionCreateDTO, assessmentId: string) {
        try {
            ...
            const existingSubmission = await AssessmentSubmission.findOne({
                ...
                'applicant.email': payload.applicant.email
            });

            if (existingSubmission) {
                logger.error(`Duplicate submission attempt by ${payload.applicant.email} for assessment ${assessmentId}`);
                throw new AppError('You have already submitted for this assessment. Submissions cannot be modified once submitted.', 400);
            }
            ...
            logger.info(`New submission created with ID: ${submission._id} for assessment: ${assessmentId}`);
            return submission
        } catch (error) {
    ...
    public async getSubmissionByEmail(assessmentId: string, email: string) {
        try {
            ...
            logger.error(`Failed to get submission by email: ${error}`);
            throw new AppError('Failed to get submission', 500);
        }
    }

    public async checkSubmissionExists(assessmentId: string, email: string): Promise<boolean> {
        try {
            const submission = await AssessmentSubmission.exists({
                assessmentId: assessmentId,
                'applicant.email': email
            });
            ...
            console.error(`Failed to check submission existence: ${error}`);
            logger.error(`Failed to check submission existence: ${error}`);
            return false;
        }
    }
}
